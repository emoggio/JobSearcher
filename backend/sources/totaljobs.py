"""Totaljobs scraper using Playwright sync API in a thread pool. Scrapes multiple pages."""
import asyncio
import logging
import urllib.parse
...
PAGES_TO_SCRAPE = 3  # 25 jobs/page → up to 75 results
    ...
    try:
        ...
        for card in cards:
            try:
                ...
                full_url = f"https://www.totaljobs.com{href}" if href and href.startswith("/") else href
                ...
                logger.debug("Totaljobs card parse error: %s", e)
        return count
    except Exception as e:
        logger.warning("Totaljobs page error: %s", e)
        return 0
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for page_num in range(1, PAGES_TO_SCRAPE + 1):
                url = (
                    f"https://www.totaljobs.com/jobs/{slug}/in-{location}"
                    f"?postedwithin=30&salary={salary_min}&page={page_num}"
                )
                found = _scrape_page(page, url, jobs, seen_urls)
                logger.debug("Totaljobs page %d: %d jobs", page_num, found)
                if found == 0:
                    break
        except Exception as e:
            logger.warning("Totaljobs scrape failed: %s", e)
        finally:
            browser.close()

    logger.info("Totaljobs: %d jobs scraped", len(jobs))
    return jobs
